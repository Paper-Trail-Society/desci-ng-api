...
async function parseCsvAndInsert(filePath: string) {
  ...
  for (const row of singleAuthorResults) {
    ...
    const [category] = await db
      ...
      .where(eq(categoriesTable.name, row.category));
    ...
    let [institution] = await db
      ...
      .where(eq(institutionsTable.name, row.institution));

    if (!institution) {
      [institution] = await db
        .insert(institutionsTable)
        .values({ name: row.institution.trim() })
        .returning();
    }
...
// TODO: handle `multipleAuthorResults` similarly
